...
public class biometricfragment extends Fragment {
    ...
    private void setupViewPager(ViewPager viewPager) {
        ...
        GenericMethods.completedbionmetric = db.getAllList(db).size();

        System.out.println("list size"+GenericMethods.todaybiometric+GenericMethods.olderbiometric+GenericMethods.newbiometric+GenericMethods.completedbionmetric);
        ...
        if (db.getAllList(db).size() > 0) {
            ...
            adapter.addFragment(new NewDetails(), "Done" + "(" + GenericMethods.completedbionmetric + ")");
        } else if (db.getAllList(db).size() == 0) {

            adapter.addFragment(new NoAppointmentdetails(), "Done" + "(" + GenericMethods.completedbionmetric + ")");
